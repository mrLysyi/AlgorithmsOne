...
public class SolverVisualizer {
    ...
    public static String timeTaken(long milliseconds) {
        double seconds = milliseconds / 1000.0;
        return " (took " + String.format("%5.3f", seconds) + " seconds)";
    }
...
//        	In in = new In(filename);
            ...
            if (!solved) {
                ...
                title += "  No solution possible" + timeTaken(now - start);
                movingTile = 0;
                moves = 0;
                ...
                if (moves != 1) title += "s";
                title += timeTaken(now - start);
                moves = 0; // reset to zero ready to display current move

                for (Board board : solver.solution()) {
                    manhattan = board.manhattan();
                    // parse board state
                    String[] numbers = board.toString().trim().split("\\s+");
